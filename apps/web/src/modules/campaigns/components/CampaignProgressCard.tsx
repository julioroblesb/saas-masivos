    ...
    async function fetchSession() {
      try {
        ...
        const { data: session } = await supabase
          .from('wa_sessions')
          .select('daily_sent_count, connection_started_at')
          .eq('company_id', profile.company_id)
          .single();
        if (session) {
          setSessionData({
            dailySentCount: session.daily_sent_count || 0,
            startedAt: session.connection_started_at,
          });
        }
      } catch (err) {
        console.error(err);
    ...
    <div className="panel w-full">
      ...
      {campaign.status === 'running' && isOutsideBusinessHours && (
        <div className="mb-4 bg-secondary-light dark:bg-secondary-dark-light border border-secondary/20 p-3 rounded-lg flex items-start gap-2">
          ...
          <p className="m-0 text-sm text-dark dark:text-white-light leading-tight">
            <strong>Fuera de horario comercial.</strong> Por tu seguridad, los envíos se han
            detenido temporalmente y se reanudarán automáticamente mañana a las 08:00 a.m. (Hora de
            Lima).
          </p>
        </div>
      )}
      ...
      {campaign.status === 'running' && !isOutsideBusinessHours && isDailyLimitReached && (
        <div className="mb-4 bg-warning-light dark:bg-warning-dark-light border border-warning/20 p-3 rounded-lg flex items-start gap-2">
          ...
          <p className="m-0 text-sm text-dark dark:text-white-light leading-tight">
            <strong>Límite de Calentamiento.</strong> Hoy ya enviaste tus {currentLimit} mensajes
            permitidos. Los envíos se pausaron por seguridad y continuarán automáticamente mañana.
          </p>
        </div>
      )}
