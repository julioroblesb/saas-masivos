...
import {
  ...
  useUpsertMarketingContact,
  useBatchInsertMarketingContacts,
  useDeleteMarketingContact,
  useDeleteMarketingContactsByTag,
...
export function MarketingContactsManager() {
  ...
  const { data: contactsData = { data: [], count: 0 }, isLoading } = useMarketingContacts(
    page,
    PAGE_SIZE,
    debouncedSearch,
  );
  ...
  const handleDeleteByTag = () => {
    ...
    if (
      window.confirm(
        `¿Estás seguro de eliminar TODOS los contactos que tengan la etiqueta "${tag}"? Esta acción no se puede deshacer.`,
      )
    ) {
  ...
  const handleAdd = () => {
    ...
    const phone = normalizePeruPhone(newPhone);
    if (!phone) {
      crmToast.error('Ingresa un celular peruano de 9 dígitos, por ejemplo 996 552 871.');
      return;
    }
    ...
    <div className="panel p-0 flex flex-col h-full overflow-hidden">
      ...
      <div className="table-responsive flex-1 overflow-auto">
        <table className="table-hover w-full min-w-[600px]">
          ...
          <tbody>
            {isLoading && (
              <tr>
                <td colSpan={5} className="p-8 text-center text-white-dark">
                  Cargando contactos...
                </td>
              </tr>
            )}
            ...
            {contacts.map((c) => (
              <tr key={c.id}>
                <td className="font-mono font-semibold text-dark dark:text-white-light">
                  {c.phone}
                </td>
                <td className="text-white-dark">
                  {c.name || <span className="text-white-dark opacity-50">—</span>}
                </td>
                <td>
                  <div className="flex gap-1.5 flex-wrap">
                    {(c.tags || []).length === 0 ? (
                      ...
                      (c.tags || []).map((tag) => (
                        <span
                          key={tag}
                          className="badge badge-outline-info flex items-center gap-1 uppercase"
                        >
                          <Tag size={9} /> {tag}
                        </span>
                      ))
                ...
                <td className="text-right">
                  <button
                    ...
                    onClick={() => {
                      if (window.confirm('¿Eliminar este contacto?')) deleteContact.mutate(c.id);
                    }}
                  >
                    <Trash2 size={14} />
                  </button>
            ...
            {totalCount > page * PAGE_SIZE && (
              <tr>
                <td colSpan={5} className="p-4 text-center">
                  ...
                  >
                    Siguiente Página ({Math.min(PAGE_SIZE, totalCount - page * PAGE_SIZE)}{' '}
                    restantes)
                  </button>
                </td>
              </tr>
      ...
      {isAddOpen && (
        <div
          ...
          onMouseDown={(e) => {
            mouseDownOnBackdropAdd.current = e.target === e.currentTarget;
          }}
          onMouseUp={(e) => {
            if (mouseDownOnBackdropAdd.current && e.target === e.currentTarget) setIsAddOpen(false);
            mouseDownOnBackdropAdd.current = false;
          }}
        >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3 col-span-1 md:col-span-2">
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  <small className="text-xs text-zinc-500 font-medium ml-1">
                    Escribe 9 dígitos; agregaremos el código 51 automáticamente.
                  </small>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        placeholder="Ej: Me escribió por Facebook, Formulario web"
                      />
                      <small className="text-xs text-zinc-500 font-medium ml-1">
                        Te ayuda a recordar el origen para evitar contactar a desconocidos.
                      </small>
                    </div>
                      ...
                      <label className="flex items-start gap-4 cursor-pointer group bg-warning/10 dark:bg-warning/5 border border-warning/20 p-5 rounded-2xl transition-colors hover:bg-warning/20">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-xs text-warning-700 dark:text-warning/80 leading-relaxed">
                            Ten en cuenta que si envías mensajes masivos a clientes
                            &quot;fríos&quot; o bases de datos compradas,{' '}
                            <strong>el riesgo de baneo por parte de WhatsApp es muy alto</strong>.
                          </span>
                        </div>
                      </label>
                    </div>
      ...
      {isBatchOpen && (
        <div
          ...
          onMouseDown={(e) => {
            mouseDownOnBackdropBatch.current = e.target === e.currentTarget;
          }}
          onMouseUp={(e) => {
            if (mouseDownOnBackdropBatch.current && e.target === e.currentTarget)
              setIsBatchOpen(false);
            mouseDownOnBackdropBatch.current = false;
          }}
        >
          <div className="panel border-0 p-0 w-full max-w-lg animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-[#e0e6ed] dark:border-[#1b2e4b]">
              ...
              <button
                className="text-white-dark hover:text-dark dark:hover:text-white-light"
            ...
            <div className="p-5 flex flex-col gap-4">
              <div className="bg-info-light dark:bg-info-dark-light p-3 rounded-lg border border-info/20">
                <p className="m-0 text-sm text-info-800 dark:text-info leading-relaxed">
                  ...
                  Formato:{' '}
                  <code className="bg-white dark:bg-dark px-1.5 py-0.5 rounded border border-info/30 font-mono text-xs">
                    Teléfono, Nombre, Etiqueta1, Etiqueta2
                  </code>
                </p>
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  placeholder={
                    '999 111 222, Juan Perez, cliente, vip\n999 333 444, Maria Lopez, lead'
                  }
              ...
              <div>
                ...
                <input
                  ...
                  value={optInSource}
                  onChange={(e) => setOptInSource(e.target.value)}
                  placeholder="Ej: Me escribieron por Facebook, Clientes de la tienda"
                />
                <small className="text-white-dark mt-1 block">
                  Te ayuda a recordar el origen para evitar contactar a desconocidos.
                </small>
              </div>
              <div className="flex items-start gap-2 bg-warning-light dark:bg-warning-dark-light p-3 rounded-lg border border-warning/20 mt-2">
                <input
                  type="checkbox"
                  id="consent-batch"
                  className="form-checkbox text-warning mt-1"
                  checked={hasOptInConsent}
                  onChange={(e) => setHasOptInConsent(e.target.checked)}
                />
                <label
                  htmlFor="consent-batch"
                  className="text-xs text-warning-800 dark:text-warning leading-snug m-0"
                >
                  <strong>
                    Confirmo que todos estos clientes me han escrito o contactado a mí primero.
                  </strong>
                  <br />
                  Ten en cuenta que si envías mensajes masivos a clientes &quot;fríos&quot; o bases
                  de datos compradas,{' '}
                  <strong>el riesgo de baneo por parte de WhatsApp es muy alto</strong>.
                </label>
              </div>
            ...
            <div className="flex items-center justify-end gap-2 p-5 border-t border-[#e0e6ed] dark:border-[#1b2e4b]">
              <button className="btn btn-outline-danger" onClick={() => setIsBatchOpen(false)}>
                Cancelar
              </button>
              <button
                className="btn btn-primary"
                onClick={handleBatch}
                disabled={!batchText.trim() || batchInsert.isPending}
              >
      ...
      {isDeleteTagOpen && (
        <div
          className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
          onMouseDown={(e) => {
            mouseDownOnBackdropDeleteTag.current = e.target === e.currentTarget;
          }}
          onMouseUp={(e) => {
            if (mouseDownOnBackdropDeleteTag.current && e.target === e.currentTarget)
              setIsDeleteTagOpen(false);
            mouseDownOnBackdropDeleteTag.current = false;
          }}
        >
          <div className="panel border-0 p-0 w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 flex flex-col gap-4">
              <div className="bg-warning-light dark:bg-warning-dark-light p-3 rounded-lg border border-warning/20">
                <p className="m-0 text-sm text-warning-800 dark:text-warning leading-relaxed">
                  Ingresa el nombre de la etiqueta. <strong>Todos los contactos</strong> que tengan
                  esta etiqueta serán eliminados de la base permanentemente.
                </p>
              </div>
              <div>
            ...
            <div className="flex items-center justify-end gap-2 p-5 border-t border-[#e0e6ed] dark:border-[#1b2e4b]">
              ...
              <button
                ...
                disabled={!deleteTagText.trim() || deleteByTag.isPending}
              >
