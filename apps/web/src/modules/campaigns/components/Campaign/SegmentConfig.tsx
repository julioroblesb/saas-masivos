...
export function SegmentConfig({
  ...
  const handleRawTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const phones = text
      .split(/[\n,;]+/)
      .map((phone) => normalizePeruPhone(phone))
      ...
      <div className="flex flex-col gap-5 sm:pl-11">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {audienceMode === 'clientes'
              ? 'Selecciona personas de tu base de clientes (CRM).'
              : 'Pega números de prospectos o listas frías.'}
          </p>
        </div>
              ...
              <div className="w-full sm:w-56">
                <CustomSelect
                  ...
                  onChange={(option: CustomSelectOption | null) =>
                    option && setServiceFilter(option.value)
                  }
                />
              </div>
            </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="border-b border-slate-200 dark:border-slate-800 sticky top-0 bg-white dark:bg-dark">
                    <tr>
                      <th className="py-3 px-2 w-10 text-center">
                        <input
                          type="checkbox"
                          checked={filteredClients.every((c) => targetContactIds.includes(c.id))}
                          onChange={(e) =>
                  ...
                  <tbody>
                    {filteredClients.map((client) => (
                      <tr
                        key={client.id}
                        className="border-t border-slate-100 dark:border-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors cursor-pointer"
          ...
          <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                ...
                <p className="text-xs text-warning dark:text-warning text-right max-w-[200px] leading-tight">
                  Los números masivos fríos tienen mayor riesgo de baneo en WhatsApp.
                </p>
              </div>
            </div>
          </div>
