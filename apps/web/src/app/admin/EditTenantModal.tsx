...
export function EditTenantModal({ company, isOpen, onClose }: EditTenantModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div role="dialog" aria-modal="true" aria-labelledby="edit-tenant-title" className="panel w-full max-w-lg border border-black-light dark:border-dark-light rounded-2xl bg-white dark:bg-dark p-0 shadow-xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        
        <div className="flex items-center justify-between p-6 border-b border-black-light dark:border-dark-light">
          <h2 id="edit-tenant-title" className="text-xl font-bold text-black dark:text-white">Gestionar Cliente: {company.name}</h2>
          <button type="button" aria-label="Cerrar" onClick={onClose} className="p-2 text-zinc-400 hover:text-black dark:hover:text-white rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
            <X size={20} />
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              ...
              <CustomSelect 
                ...
                onChange={(option: CustomSelectOption | null) => option && handlePlanChange(option.value)}
                options={[
            ...
            <div>
              ...
              <CustomSelect 
                ...
                onChange={(option: CustomSelectOption | null) => option && setStatus(option.value)}
                options={[
