...
create index if not exists on crm_marketing_contacts (company_id);
create index if not exists on crm_marketing_contacts using gin (tags);
...
create index if not exists on crm_wa_queue (campaign_id, status);
create index if not exists on crm_wa_queue (company_id, status);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_crm_wa_queue_scheduled ON crm_wa_queue (scheduled_for) WHERE status = 'pendiente';
...
create index if not exists if not exists crm_wa_queue_company_phone_sent_idx on crm_wa_queue (company_id, phone, sent_at desc);

-- Bot configurations
ALTER TABLE companies ADD COLUMN IF NOT EXISTS bot_prompt text DEFAULT 'Eres un asistente útil y amable.';
ALTER TABLE companies ADD COLUMN IF NOT EXISTS bot_enabled boolean DEFAULT false;
