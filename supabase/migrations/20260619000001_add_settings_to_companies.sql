...
-- The original repository had a second migration with the same version.
-- It is consolidated here so every migration version remains unique.
...
CREATE OR REPLACE FUNCTION rpc_archive_contacts(
    p_contact_ids uuid[],
    p_archive boolean
) RETURNS void AS $$
DECLARE
